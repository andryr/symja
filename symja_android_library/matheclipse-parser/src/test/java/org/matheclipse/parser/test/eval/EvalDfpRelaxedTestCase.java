...
public class EvalDfpRelaxedTestCase extends TestCase {
	...
	public void testEval004() {
		try {
			DfpEvaluator engine = new DfpEvaluator(50, true);
			IDfpValue vd = engine.defineVariable("X", 3.0);
	...
	public void testEval006() {
		try {
			...
			assertEquals(d.getReal(), 1d, DoubleEvaluator.EPSILON);
			vb.setValue(false);
			d = engine.evaluate();
			assertEquals(d.getReal(), 0d, DoubleEvaluator.EPSILON);
		} catch (Exception e) {
			e.printStackTrace();
			assertEquals("", e.getMessage());
		}
	...
	public void testEval007() {
		try {
			...
			IDfpValue vd = engine.defineVariable("$1", 3.0);
			IDfpValue vd2 = engine.defineVariable("$2", -4.0);
	...
	public void testEval008() {
		try {
			...
			assertEquals(list.toString(), "[b, a, $c]");
		} catch (Exception e) {
			e.printStackTrace();
			assertEquals("", e.getMessage());
		}
